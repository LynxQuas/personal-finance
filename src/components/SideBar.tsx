...
const navItems = [
    ...
    { label: "Recurring Bills", href: "/dashboard/recurring-bills", icon: RecurringBills },
]
            ...
            <ul>
                {navItems.map((nav) =>
                    <li className="pr-6" key={nav.label} >
                        <Link className="text-gray-300 cursor-pointer block py-4 px-7"
                            to={nav.href}
                            activeOptions={{ exact: true }}
                            activeProps={
                                {
                                    className: 'rounded-md ml-0 text-black! block py-4 px-[22px] border-l-secondary-green border-l-6 bg-beige-100 '
                                }
                            }
                        >
                            {({ isActive }) => {
